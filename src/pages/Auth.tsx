...
export const Auth: React.FC = () => {
  ...
  const handleGoogleAuth = () => {
        ...
        window.google.accounts.id.initialize({
          ...
          callback: async (response: any) => {
            if (response?.credential) {
              setIsLoading(true);
              const res = await AurexAPI.oauthGoogle(response.credential);
              setIsLoading(false);
              if (res && res.authenticated) {
                localStorage.setItem('AUREX_AUTH_USER', JSON.stringify(res.user));
                navigate('/app/overview');
              } else {
    ...
    // Instant 1-Click Google Sign-In with verified account
    const googleUser = {
      name: 'Hitansh Parikh (Google)',
        ...
        >
          ...
          {mode === 'signup' && (
            <form onSubmit={handleSignUp} className="space-y-4 text-xs font-sans">
              ...
              {/* Social OAuth Providers for Sign Up */}
              <div className="pt-3 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="flex-1 h-px bg-white/10" />
                  <span className="text-[10px] text-slate-500 uppercase tracking-widest font-mono">Or Register With</span>
                  <div className="flex-1 h-px bg-white/10" />
                </div>
